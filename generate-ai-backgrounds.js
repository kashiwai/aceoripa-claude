...
// AI背景生成スクリプト作成
function createAIBackgroundScript() {
  const scriptContent = `
import os
...
# プロンプトリスト
prompts = ${JSON.stringify(backgroundPrompts, null, 2)}
...
`;
  ...
  fs.writeFileSync('generate-ai-backgrounds.py', scriptContent);
  console.log('✅ AI背景生成スクリプト作成完了');
}

// 背景画像リストを読み込む
function loadBackgroundsList() {
  try {
    const data = fs.readFileSync('ai-backgrounds-list.json', 'utf8');
    return JSON.parse(data);
  } catch (err) {
    console.log('⚠️  背景リストが見つかりません。デフォルト背景を使用します。');
    return [];
  }
}
...
`;
  ...
  fs.writeFileSync('composite-banner-example.js', compositeScript);
  console.log('✅ コンポジット例作成完了');
}

// メイン実行
function main() {
  console.log('🚀 AI背景生成システム準備\n');
  
  // スクリプト作成
  createAIBackgroundScript();
  createCompositeExample();
  
  console.log('\n📝 使い方:');
  console.log('1. python generate-ai-backgrounds.py でAI背景を生成');
  console.log('2. 生成された背景画像は public/images/ai-backgrounds/ に保存');
  console.log('3. composite-banner-example.js を参考に背景とカード/文字を合成');
  
  console.log('\n💡 ヒント:');
  console.log('- 背景は一度生成すれば何度でも再利用可能');
  console.log('- 異なる文字やカード配置で多数のバリエーション作成可能');
  console.log('- 背景の明暗に応じて文字色を調整すると見やすい');
}
